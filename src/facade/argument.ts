...
export class FacadeArgument {
    ...
    async getTreeNode(id: number, max_depth: number, current_depth: number) {
        ...
        console.log(`Getting tree for argument ${id} with ${arg.premises.length} premises`);
        ...
        if (current_depth < max_depth)
        {
            ...
            children = await Promise.all(arg.premises.map(async (statement) => this.fs.getTreeNode(statement.id, max_depth, current_depth + 1)));
/*
